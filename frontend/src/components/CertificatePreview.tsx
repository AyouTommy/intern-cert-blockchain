            ...
            <div className="relative h-full flex flex-col px-8 py-6">
                ...
                <div className="text-center mb-4 space-y-2">
                    ...
                    <p className="text-sm text-gray-600">
                        于 {format(new Date(certificate.startDate), 'yyyy.MM.dd')} - {format(new Date(certificate.endDate), 'yyyy.MM.dd')}
                    </p>
                    <p className="text-sm text-gray-600">
                        在 <span className="font-medium text-indigo-600">{certificate.company.name}</span> 完成实习
                    </p>
                    <p className="text-sm text-gray-500">
                        岗位: {certificate.position}
                    </p>
                </div>
                    ...
                    >
                        ...
                        <div className="font-mono text-xs text-gray-600 space-y-1.5">
                            <div className="flex">
                                ...
                                <span className="text-indigo-600 break-all">{certificate.certHash}</span>
                            </div>
                            {certificate.txHash && (
                                <div className="flex">
                                    <span className="text-gray-400 w-20 flex-shrink-0">Tx Hash:</span>
                                    <span className="text-indigo-600 break-all">{certificate.txHash}</span>
                                </div>
                            )}
                            <div className="flex gap-6 mt-2 pt-2 border-t border-indigo-100">
                                <span>
                                    <span className="text-gray-400">Block: </span>
                                    <span className="font-semibold text-gray-700">#{certificate.blockNumber?.toLocaleString()}</span>
                                </span>
                                <span>
                                    <span className="text-gray-400">Chain ID: </span>
                                    <span className="font-semibold text-gray-700">{certificate.chainId}</span>
                                </span>
                            </div>
                        </div>
                    </div>
